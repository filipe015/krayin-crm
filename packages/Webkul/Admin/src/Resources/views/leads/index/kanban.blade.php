                                ...
                                >
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div class="flex items-center gap-1">
                                            ...
                                            <!-- WhatsApp Shortcut -->
                                            <span
                                                class="flex cursor-pointer items-center rounded p-1 transition-all hover:bg-gray-200 dark:hover:bg-gray-800"
                                                v-if="element.person?.contact_numbers?.length"
                                                @click.stop.prevent="openWhatsApp(element)"
                                                title="{{ trans('admin::app.leads.index.kanban.whatsapp') }}"
                                            >
                                                <svg viewBox="0 0 32 32" class="h-4 w-4" fill="#25D366">
    ...
    <script type="module">
        app.component('v-leads-kanban', {
            ...
            methods: {
                ...
                /**
                 * Strips everything but digits from a phone number and ensures the
                 * Brazil country code (55) is present, so it can be used in a `wa.me`
                 * link (which only accepts country code + digits).
                 *
                 * @param {string} phone
                 * @returns {string}
                 */
                ...
                /**
                 * Opens WhatsApp in a new tab using the lead's primary contact number.
                 *
                 * @param {object} element
                 * @returns {void}
                 */
                openWhatsApp(element) {
                    const phone = element.person?.contact_numbers?.[0]?.value;

                    if (! phone) {
                        return;
                    }

                    window.open(`https://wa.me/${this.cleanPhoneNumber(phone)}`, '_blank');
                },
            }
        });
    </script>
